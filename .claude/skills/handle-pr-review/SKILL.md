...
description: Fetch PR review comments, triage them (fix, challenge, or skip), apply holistic fixes, resolve threads, and post a summary.
disable-model-invocation: true
argument-hint: "[pr] [auto=true|false]"
---
...
<!-- sync: canonical version at JesusFilm/core:.claude/commands/handle-pr-review.md — keep triage categories and the summary comment step aligned when upstream changes. -->
<!-- Vendored from JesusFilm/youtube-studio and repointed at this repo: default
     owner/repo, the tooling in §5, and commit discipline are cadent's. Not
     managed by skills-lock.json (that tracks the mattpocock/skills set), so a
     skills sync will not update it — re-pull by hand when upstream moves. -->
...
Default owner/repo is **`mikeallisonJS/cadent`**. Infer from `git remote -v` when run inside a clone (`gh` does this automatically for most commands). Use the base repo's `owner`/`name` in GraphQL and REST API calls (works across forks).
...
gh api graphql -f query='
  ...
  }' -f owner=mikeallisonJS -f repo=cadent -F pr=NUMBER
```
...
Python managed with **uv**; the venv is `.venv`. See [AGENTS.md](../../../AGENTS.md).

```bash
uv run pytest         # the suite (no audio/GPU needed)
uv run ruff check .   # lint — line-length 100, see [tool.ruff] in pyproject.toml
```

Target a single file when appropriate: `uv run pytest tests/test_config.py`.

**Platform seam.** Per-OS behaviour lives behind `cadent/platform/` (ADR 0005), and CI runs the suite on `windows-latest` *and* `macos-14`. A fix that reaches for `sys.platform` outside an adapter, or that hardcodes copy naming a Windows-only setting, will pass locally and fail the other leg — put the fact in `Capabilities` instead. Adapter-internal tests platform-skip themselves, so a green local run on one OS is not proof.

**Changelog fragment.** Any change touching `cadent/`, `packaging/`, `scripts/`, or `pyproject.toml` needs a `changelog.d/<issue>.<type>.md` file or the `changelog-check` job hard-fails. If the fix round adds code changes to a PR that already has a fragment, amend that fragment rather than adding a second. See `docs/agents/releases.md`.

### Commit discipline

- One commit per logical change. Subjects are plain sentences in the imperative ("Stop calling Cadent a Windows-only app"), not conventional-commit prefixes — match `git log`.
- Group related changes into a single commit rather than one commit per comment.
- Reference the feedback being addressed in the commit message body, including any comment you deliberately did *not* apply and why.
- **Stage explicitly.** Name the paths you changed. `git add -A` here can sweep up a parallel session's in-flight work and build artefacts that `.gitignore` doesn't cover (generated icons under `packaging/icons/`, for one) — check `git status` before staging, and `git log -1 --stat` after committing.
...
```bash
gh api repos/mikeallisonJS/cadent/pulls/PR/comments/DATABASE_ID/replies \
...
```bash
gh api repos/mikeallisonJS/cadent/issues/PR/comments -f body="Your reply here"
```
